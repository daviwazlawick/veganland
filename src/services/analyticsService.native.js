...
function safeLog(name, params, value) {
  if (!configured || !initialized) return;
  try {
    if (typeof value === 'number' && !Number.isNaN(value)) {
      AppEventsLogger.logEvent(name, value, params || {});
    } else {
      AppEventsLogger.logEvent(name, params || {});
    }
  } catch (e) {
    if (__DEV__) console.warn('FB log failed', name, e);
  }
}
...
export function logStartTrial({ price, currency, planId } = {}) {
  ...
  try {
    ...
    if (__DEV__) console.warn('FB StartTrial failed', e);
  }
}
...
export function logSubscribe({ price, currency, planId } = {}) {
  ...
  try {
    ...
    if (__DEV__) console.warn('FB Subscribe failed', e);
  }
}
