...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && req.url === '/app/version') {
      ...
      sendJson(res, 200, {
        ios:     { min: '1.0.18', store_url: iosStore },
        android: { min: '1.0.18', store_url: androidStore },
        web:     { min: '1.0.0' },
      }, origin);
      return;
    }
    ...
    if (req.method === 'GET' && req.url === '/auth/me') {
      ...
      const [user, usage, streak] = await Promise.all([
        getUserById(claims.userId),
        getScanUsage(claims.userId),
        getUserStreak(claims.userId),
      ]);
      if (!user) {
    ...
    if (req.method === 'GET' && req.url === '/nutrition/profile') {
      ...
      if (profile) {
        ...
        profile.bmr = bmr;
        profile.tdee = bmr != null ? Math.round(bmr * (multipliers[profile.activity_level] || 1.375)) : null;
      }
    ...
    if (req.method === 'POST' && req.url === '/body/analyze') {
      ...
      try {
        ...
        const result = await new Promise((resolve, reject) => {
          ...
          py.on('close', code => {
            clearTimeout(timer);
            // Always surface Python stderr — filter out MediaPipe/protobuf noise
            const errLines = err.split('\n').filter(l =>
              l && !l.includes('SymbolDatabase') && !l.includes('deprecated')
              && !l.includes('WARNING') && !l.includes('TensorFlow')
              && !l.match(/^\s*$/));
            if (errLines.length) console.error('[body-analysis] stderr:\n' + errLines.join('\n'));
            if (code !== 0) {
              let msg = 'analysis failed';
              try { const p = JSON.parse(out); if (p?.error) msg = p.error; } catch {}
              console.error('[body-analysis] exit', code, '| out:', out.slice(-500));
    ...
    // GET /nutrition/plates — recent plate analysis logs
    if (req.method === 'GET' && req.url === '/nutrition/plates') {
      const claims = getAuthUser(req);
      if (!claims) { sendJson(res, 401, { error: 'Unauthorized' }, origin); return; }
      const rows = await getRecentPlateLogs(claims.userId);
    ...
    if (req.method === 'POST' && req.url === '/webhook/revenuecat') {
      ...
      if (event) {
        ...
        if (newUserType && userId) {
          ...
          if (eventType === 'INITIAL_PURCHASE' && (newUserType === 'starter' || newUserType === 'premium')) {
            grantReferralSignupBonusOnPurchase(userId, newUserType).catch(console.error);
          }
        }
      }
